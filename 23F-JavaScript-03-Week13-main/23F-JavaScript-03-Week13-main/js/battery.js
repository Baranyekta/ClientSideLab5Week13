...
// STEP 5: grabbing the image element to display RoboHash image
...
// STEP 6: function to fetch a RoboHash image based on users battery %
function fetchRoboImage(batteryPercentage) {
    const apiUrl = `https://robohash.org/${batteryPercentage}.png`;
    ...
    fetch(apiUrl, {
        ...
        .catch(error => console.error('Error fetching RoboHash image:', error));
}
...
navigator.getBattery().then(battery => {
    ...
    // STEP 7: calling fetchRoboImage function to display image
    const batteryPercentage = Math.round(battery.level * 100);
    fetchRoboImage(batteryPercentage);
    ...
    battery.addEventListener("levelchange", () => {
        updateBatteryStatus(battery);
        // updating RoboHash image when the users device charge  level changes
        const batteryPercentage = Math.round(battery.level * 100);
        fetchRoboImage(batteryPercentage);
    });
})
